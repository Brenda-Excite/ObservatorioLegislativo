...
const ModalLegislature = ({ open, handleClose }: Props) => {
  ...
  const onSubmit: SubmitHandler<Inputs> = (data) => {
    dispatch(startUpdatedLegislature(data));
    ...
    <Modal show={open} onHide={handleClose}>
      <Form onSubmit={handleSubmit(onSubmit)}>
        <Modal.Header closeButton>
          <Modal.Title> {"Editar Legislatura"}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Form.Group className="mb-3">
            ...
            />
            {errors.name && <p className="text-danger">El nombre de la legislatura es requerida</p>}
          </Form.Group>
        </Modal.Body>
        <Modal.Footer>
